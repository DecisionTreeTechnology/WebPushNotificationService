...
/opt/mssql-tools/bin/sqlcmd -S sqlserver -U SA -P $SA_PASSWORD -Q "CREATE DATABASE [$WebPush_DB]"

/opt/mssql-tools/bin/sqlcmd -d $AuthServer_DB -S sqlserver -U sa -P $SA_PASSWORD -i migrations-AuthServerHost.sql
/opt/mssql-tools/bin/sqlcmd -d $WebPush_DB -S sqlserver -U sa -P $SA_PASSWORD -i migrations-WebPush.sql